...
sub make_fixtures : Test(setup) {
    ...
    $self->{file} = Code::Statistics::File->new(
        collector     => $self->{collector},
        original_path => 'data/json/basic_collect.json',
        path          => 'data/json/basic_collect.json'
    );
...
sub _format_file_path :
  TestGroup(file path formatting works with different inputs) {
    my ($self) = @_;
